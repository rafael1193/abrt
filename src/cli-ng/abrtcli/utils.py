...
def fmt_problems(probs, fmt=MEDIUM_FMT):
    ...
    for prob in probs:
        ...
        if uid:
            try:
                uid_username = ('{0} ({1})'
                                .format(uid, pwd.getpwuid(int(uid)).pw_name))
            except KeyError:
                uid_username = str(uid)

            context_vars['uid_username'] = uid_username

        if prob.non_reportable:
            context_vars['non_reportable'] = 'Not reportable'
            reason = prob.non_reportable_reason.rstrip()

            if tabular:
                reason = reason.replace('\n', '\n,')

            context_vars['non_reportable_reason'] = reason
